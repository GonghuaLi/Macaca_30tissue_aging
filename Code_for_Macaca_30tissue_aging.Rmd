...
class_abbr <- c("Juvenile" = "Juv", "Young" = "Yng","Elderly" = "Eld")
...
for (i in 1:length(utissue)) {
  ...
  tmpdata$class <- class_abbr[tmpdata$class]
  tmpdata$class <- factor(x = tmpdata$class,
    levels = c("Juv", "Yng", "Eld"))
  tmpp1 <- t.test(Area.Density ~ class, data =
    tmpdata, subset = tmpdata$class != "Yng")$p.value
  tmpp2 <- t.test(Area.Density ~ class, data =
    tmpdata, subset = tmpdata$class != "Juv")$p.value
  ...
  p <- ggplot(df2, aes(x = class, y = tmean, fill = class)) +
    ...
    geom_errorbar(aes(ymin = tmean - tsd, ymax = tmean + tsd),
      width = .5, size = 1.5) +
    geom_jitter(data = tmpdata, aes(x = class, y = Area.Density.log2),
              width = 0.25, , height = 0, size = 6, shape = 21,
              color = "black", fill = "#FFFFFF99", stroke = 0.8) +
    lghplot.addtheme(size = 30, sizex = 30, sizey = 30) +
...
writetxt(tpvalues,'./results/experiments/tpvalues_p16.txt')
tpvalues
```
...
for (i in 1:length(utissue)) {
  ...
  tmpp1 <- t.test(Area.Density ~ class, data = tmpdata,
    subset = tmpdata$class != "Yng")$p.value
  tmpp2 <- t.test(Area.Density ~ class, data = tmpdata,
    subset = tmpdata$class != "Juv")$p.value
  tpvalues$p_Juvenile_vs_Elderly[i] <- tmpp1
  tpvalues$p_Young_vs_Elderly[i] <- tmpp2
...
writetxt(tpvalues,'./results/experiments/tpvalues_p21.txt')
tpvalues
```
...
for (i in 1:length(utissue)) {
  ...
  thepath <- paste0("./results/experiments/Extendfigure_HE_cellcounts_",
    utissue[i], ".pdf")
  pdf(thepath,height = 5.2,width = 5.2)
  p <- ggplot(df2, aes(x = class, y = tmean, fill = class)) +
    geom_bar(stat = "identity", color = "black",
      position = position_dodge(), alpha = 0.5, size = 1.5) +
    geom_errorbar(aes(ymin = tmean - tsd, ymax = tmean + tsd),
      width = .5, size = 1.5) +
    geom_jitter(data = tmpdata, aes(x = class, y = number),
              width = 0.25, , height = 0, size = 6, shape = 21,
              color = "black", fill = "#FFFFFF99", stroke = 0.8) +
    lghplot.addtheme(size = 30, sizex = 30, sizey = 30) +
    scale_fill_manual(values = c("#008B45FF", "#3B4992FF", "#EE0000FF")) +
    theme(axis.line = element_line(size = 1.2)) + xlab("") +
    ylab("# of parenchymal cells") + ggtitle(utissue[i])
  print(p)
  dev.off()
}
writetxt(tpvalues,'./results/experiments/tpvalues_cellcounts.txt')
tpvalues
```
